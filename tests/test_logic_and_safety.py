...
async def test_record_failure_escalation():
    controller = SafetyController()
    for i in range(5): await controller.record_failure("API", {})
...
def test_can_trade_new_insufficient_capital():
    ...
    allowed, _ = governor.can_trade_new(300000, {"strategy": "ENTRY"})
    assert allowed == False

def test_can_trade_new_hedge_allowed():
    governor = CapitalGovernor(total_capital=1000000)
    governor.update_state(margin=900000, count=9)
    allowed, _ = governor.can_trade_new(200000, {"strategy": "HEDGE"})
    assert allowed == True
...
async def test_evaluate_portfolio_delta_breach(test_settings):
    ...
    portfolio_risk = {"aggregate_metrics": {"delta": 0.50}} 
    market = {"spot": 21500}
...
def test_validate_snapshot_valid():
    ...
    is_valid, _ = gate.validate_snapshot({"spot": 21500.50, "vix": 14.2})
    assert is_valid == True

def test_validate_snapshot_zero_spot():
    gate = DataQualityGate()
    is_valid, _ = gate.validate_snapshot({"spot": 0.0, "vix": 14.2})
    assert is_valid == False

def test_validate_snapshot_negative_vix():
    gate = DataQualityGate()
    is_valid, _ = gate.validate_snapshot({"spot": 21500.0, "vix": -1.0})
    assert is_valid == False
